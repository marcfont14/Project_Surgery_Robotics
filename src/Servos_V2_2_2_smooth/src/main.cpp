...
void moveServos() {
  ...
  // Smaller α = smoother but slower response.
  const float alpha = 0.2;

  // --- Smooth RPY values ---
  roll  = (1 - alpha) * roll  + alpha * Gri_roll;
  pitch = (1 - alpha) * pitch + alpha * Gri_pitch;
  yaw   = (1 - alpha) * yaw   + alpha * Gri_yaw;
